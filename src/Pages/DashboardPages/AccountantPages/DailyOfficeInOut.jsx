...
const DailyOfficeInOut = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const { data } = await axiosSecure.get(`/get-daily-ledger?date=${today}`);
      return data;
    },
  });
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-green-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <select
                  {...register("category", {
                    required: "ক্যাটাগরি সিলেক্ট করুন",
                  })}
              ...
              <button
                type="submit"
                disabled={isPosted}
                className={`w-full text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition shadow-lg 
                ...
                <div className="p-4 bg-gray-50 border-b flex flex-col md:flex-row justify-between items-center">
                  <h3 className="font-semibold text-gray-700 flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500">
                      ({format(new Date(today), "MMMM dd, yyyy")})
                    </span>
                  </h3>
                  <button
                    onClick={handlePrint}
                      ...
                      {dailyEntries.length > 0 ? (
                        dailyEntries.map((logs, idx) => (
                          <tr>
                            ...
                            <td className="p-4 sm:text-[8px] md:text-sm text-right text-green-700">
                              ৳ {logs?.type === "income" ? logs?.amount : "--"}
                            </td>
                            <td className="p-4 sm:text-[8px] md:text-sm text-right text-red-600 font-bold">
                              ৳ {logs?.type === "expense" ? logs?.amount : "--"}
                            </td>
                          </tr>
                        ))
                      ) : (
        ...
        <div id="printable-ledger" className="hidden print:block">
          <div className="text-center border-b-2 pb-4 mb-4">
            ...
            <p className="font-sans font-semibold">
              তারিখ: {format(new Date(today), "MMMM dd, yyyy")}
            </p>
          </div>
          ...
          <div className="mt-4 text-right">
            <p className="text-lg font-bold">
              অবশিষ্ট ক্যাশ: ৳ {summary.netBalance}
            </p>
          </div>
